// Creating a self-invoking function run immediately after its definition. The 2D game board.
const gameboard = (function () {
    const board = [];
    // Creating a for loop to iterate 3 times for 3 rows of the board. During each iteration, an empty sub-array is added to the board array.
    // The inner loop is the columns
    for (let i = 0; i < 3; i++) {
        board.push([]);
        for (let j = 0; j < 3; j++) {
            board[i].push(null);
    ...
    // Flat representation of the board.
    function getBoard() {
    ...
    function updateCell(x, y, mark) {
        if (board[x][y] != null) {
            ...
            board[x][y] = mark;
            return 1; // Indicates success if mark can be placed and is not already taken. Mark being "X" or "O"
        }
    }
    ...
    function checkWin() {
        ...
        for (const row of board) {
            if ((row.every((cell) => (cell !== null && cell === row[0]))) === true) { // Check if every cell is equal, if condition is met return true
...
const game = (function () {
    ...
    function roundStart() {
        ...
        if (round > 9) {
            display.updateDisplay("It's a tie!");
...
const display = (function () {
    ...
    function resultHighlight(direction, index = null) {
        switch(direction){
            ...
            case "col":
                for (let i = 0; i < 3; i++) {
                    cells[index * 3].classList.add("winner");
                }
                break;
            case "diag1":
                for (let i = 0; i < 3; i++) {
